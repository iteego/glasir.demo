...
dojo.declare("dojox.image._LightboxDialog",
  ...
  //Initial value is set to blank.gif to workaround problem in IE when
  // 404 HTTP error is raised when imgUrl set to null or empty string
  ...
  show: function(/* Object */groupData){
    ...
    if(!_t._imageReady || _t.imgNode.complete === true){
      ...
      // code to re-set the src after onload connection made:
      if(dojo.isIE){ _t.imgNode.src = _t.imgNode.src; }

    }else{
      // do it quickly. kind of a code, but image is ready now
  ...
  hide: function(){
    ...
    dojo.fadeOut({node:this.titleNode, duration:5,
