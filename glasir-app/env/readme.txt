...
BigEar
A "big" .ear file contains all code for all deployment configurations, where a configuration is a combination of targets (storefront, fulfillment, ...) and environments (test, dev, production, ...).

Configuration
To enable a module in a configuration, add it to the ATG-Requires section of the MANIFEST.MF file of the configuration.
E.g. to add "MyAtgModule" to the "env.storefront.production" configuration, add it to the ATG-Requires line in the file "env/storefront/production/META-INF/MANIFEST.MF" like this (no quotes): 
"ATG-Required: env.storefront MyAtgModule"
...
E.g. "startDynamo -m env.storefront.production"
